...
let action;
const actions = ['model', 'redux'];
if (commandLength > 3) moduleName = process.argv[2];
if (commandLength === 4) {
  action = process.argv[3];
  if (!actions.includes(action)) console.log('Invalid command');
  if (action === 'model') {
    Model();
  }
  if (action === 'redux') {
...
const reduxHelper = async () => {
  ...
  const rootDefinitionImport = `import ${name}Reducer from 'store/reducers/${name}.reducer'; \n// _RI_`;
